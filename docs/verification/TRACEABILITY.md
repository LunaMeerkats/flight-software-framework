...
synchronous start/work/stop/in-place-restart, and bounded application messaging.
...
one-message dispatch together verify RFF-REQ-003. Returned-error containment
remains partial because no structured failure event exists. Implementation and
verification state remain separate so planned evidence is not represented as
completed behavior.
