...
a standalone bounded structured-event queue plus an injected manual clock.
Public integration tests satisfy the complete two-application RFF-REQ-002
scenario. The routing core, lifecycle-owned availability, and caller-selected
one-message dispatch together verify RFF-REQ-003. RFF-REQ-004 remains partial
because no scheduled work exists. RFF-REQ-005 remains partial because the clock
capture seam is not owned by a runtime/application or host integration.
...
| RFF-REQ-004 | Partial: ordered elapsed `FrameworkInstant`, object-safe injected `Clock`, explicit `ManualClock` advancement, typed non-mutating overflow, and identical manual instant traces; no scheduled work, equal-deadline ordering, or replayed scheduled trace | Preserve clock regressions; add caller-driven due-work selection, stable equal-deadline order, and a replayed scheduled-work trace without wall-clock sleeps | `manual_clock_starts_at_origin_and_reads_do_not_advance`; `manual_clock_advances_only_by_explicit_durations`; `advance_overflow_is_typed_and_preserves_current_time`; `identical_manual_sequences_produce_identical_traces`; `cargo test --workspace --all-features` (47 passed) | `0ec684f6d50505aea67acef27e54d7175f0a11ee` | Partially verified |
| RFF-REQ-005 | Partial: typed framework/application source, severity, mission-defined copied identifier, elapsed timestamp, standalone positive-capacity FIFO queue with caller-visible reject-newest saturation, and injected clock capture; no runtime/application emission or host scenario owns that composition | Preserve structured-field, clock-capture, exact-capacity, FIFO, and saturation regressions; add runtime/host-owned clock-captured event emission | `event_exposes_structured_fields_without_text_parsing`; `queue_requires_positive_capacity`; `queue_accepts_its_exact_limit_and_dequeues_fifo`; `reject_newest_preserves_older_events_and_allows_explicit_retry`; `injected_clock_timestamps_events_without_changing_queue_policy`; `cargo test --workspace --all-features` (47 passed) | `0ec684f6d50505aea67acef27e54d7175f0a11ee` | Partially verified |
